...
//  Maps
//
//  Created by Wei Kang Tan on 18/09/2024.
//
...
extension MapViewController: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, didSelect annotation: MKAnnotation) {
        ...
        if let annotation = annotation as? AreaAnnotation {
            ...
            let parkingSpotModel = annotation.parkingSpotModel
...
extension MapViewController: MapManagerDelegate{
    func didFetchAreaData(_ areasModel: [AreaModel]) {
        ...
        guard let areas = areas else {
            print("NO Area Data fetch")
            return
        }
        
        for area in areas {
            addAreaAnnotation(title: area.areaName, availableParking: area.availableParkingString, latitude: area.latitude, longtitude: area.longtitude, area: area)
        }
    ...
    func didFetchStreetAndParkingSpotData(_ streetsModel: [StreetModel]) {
        DispatchQueue.main.async { [self] in
            if let areaDetailFloatingPanel = areaDetailFloatingPanel {
                areaDetailFloatingPanel.streetModels = streetsModel
                // Set the initial detent to .medium, so it starts at half height
                present(areaDetailFloatingPanel, animated: true, completion: nil)
            }
        }
    }
    ...
    func didFetchParkingSpotData(_ parkingSpotModel: [ParkingSpotModel]){
        DispatchQueue.main.async { [self] in
            ...
            contentVC.availableParkingSpotModel = parkingSpotModel
...
extension MapViewController: StreetInformationViewControllerDelegate {
    func parkingSpotRowDidSelected(_ parkingSpotModel: ParkingSpotModel) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
            ...
            let annotation = ParkingAnnotation(patkingLotID: stringParkingLotID, coordinate: parkingLocation, parkingSpotModel: parkingSpotModel)
...
extension MapViewController: NearestParkingViewControllerDelegate {
    func didSelectedNearestParking(_ nearestParkingModel: ParkingSpotModel) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
            ...
            let annotation = ParkingAnnotation(patkingLotID: stringParkingLotID, coordinate: parkingLocation, parkingSpotModel: nearestParkingModel)
            self.mapView.addAnnotation(annotation)
            self.mapView.selectAnnotation(annotation, animated: true)
